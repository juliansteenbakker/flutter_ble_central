...
Thanks for considering contributing to flutter_ble_central!
...
The plugin is a thin wrapper around each platform's native BLE central API, so most
behaviour (scan throttling, permission prompts, advertisement coalescing) is dictated
by the OS rather than by this package.

Layout:

* `lib/` — the public Dart API and the model classes.
* `android/src/` — Kotlin plugin implementation (`BluetoothLeScanner`).
* `darwin/flutter_ble_central/Sources/` — Swift implementation shared by iOS and
  macOS (`CoreBluetooth`).
* `windows/` — C++ plugin implementation (WinRT `Bluetooth*` APIs).
...
3. `darwin/flutter_ble_central.podspec` is version-bumped automatically as an extra
